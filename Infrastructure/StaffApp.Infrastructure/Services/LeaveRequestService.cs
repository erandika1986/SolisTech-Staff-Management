...
namespace StaffApp.Infrastructure.Services
{
        ...
        IConfiguration configuration) : ILeaveRequestService
    {
        ...
        public async Task<GeneralResponseDTO> CreateLeaveRequestAsync(EmployeeLeaveRequestDTO leaveRequest)
        {
            ...
            for (int i = 0; i < leaveRequest.Files.Count; i++)
            {
                var extension = Path.GetExtension(leaveRequest.Files[i].Name);

                var fileName = $"{Path.GetFileNameWithoutExtension(leaveRequest.Files[i].Name)}{extension}";
                var uniqueFileName = $"{Guid.NewGuid()}{extension}";
                var filePath = Path.Combine(leaveSupportDocumentPath, uniqueFileName);

                await using var stream = leaveRequest.Files[i].OpenReadStream(maxFileSize);
                using var memoryStream = new MemoryStream();
                await stream.CopyToAsync(memoryStream);
                memoryStream.Position = 0;

                var uploadedFileUrl = await azureBlobService
                    .UploadFileAsync(memoryStream, uniqueFileName, leaveRequest.Files[i].ContentType, ApplicationConstants.AzureBlobStorageName);
        ...
        public async Task<EmployeeLeaveRequestDTO> GetLeaveRequestById(int leaveRequestId)
        {
        ...
        public async Task<GeneralResponseDTO> RemoveSavedSupportFile(EmployeeLeaveRequestSupportFileDTO file)
        {
            try
            {
