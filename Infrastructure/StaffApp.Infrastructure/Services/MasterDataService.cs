...
namespace StaffApp.Infrastructure.Services
{
        ...
        IRoleService roleService) : IMasterDataService
    {
        ...
        public List<DropDownDTO> GetEmployeeSalaryStatus(bool hasDefaultValue = false)
        {
            var response = new List<DropDownDTO>();
            if (hasDefaultValue)
                response.Add(new DropDownDTO() { Id = 0, Name = "All" });

            response.AddRange(EnumHelper.GetDropDownList<EmployeeSalaryStatus>());

            return response;
        }

        public List<DropDownDTO> GetProjectStatus(bool hasDefaultValue = false)
        {
            var response = new List<DropDownDTO>();
            if (hasDefaultValue)
                response.Add(new DropDownDTO() { Id = 0, Name = "All" });

            response.AddRange(EnumHelper.GetDropDownList<ProjectStatus>());
        ...
        {
            return EnumHelper.GetDropDownList<Month>();
        }
    }
}
